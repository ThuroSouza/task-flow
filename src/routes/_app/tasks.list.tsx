...
import { useTasks, useColumns, useClients, useProfiles, useSubtasks, type Task } from "@/hooks/use-data";
...
function ListPage() {
  ...
  useEffect(() => {
    ...
    if (t) {
      setEdit(t);
      setOpen(true);
      navigate({ to: "/tasks/list", search: (p: any) => ({ ...p, task: undefined }), replace: true });
    }
  ...
  const list = useMemo(() => {
    const r = applyTaskFilters(tasks, filters, { userId: user?.id ?? null, subtaskAssigneeTaskIds, subtaskAssigneeTaskIdsByUser });
    return [...r].sort((a, b) => {
  ...
  return (
    <div className="space-y-4 p-6">
      <header className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <Button onClick={() => { setEdit(null); setOpen(true); }}><Plus className="mr-2 h-4 w-4" />Nova tarefa</Button>
      </header>
      ...
      <div className="overflow-hidden rounded-lg border bg-card">
        <table className="w-full text-sm">
          ...
          <tbody>
            {list.length === 0 && (
              <tr><td colSpan={6} className="py-10 text-center text-muted-foreground">Nenhuma tarefa</td></tr>
            )}
            {list.map(t => {
              ...
              return (
                <tr key={t.id} className="cursor-pointer border-t hover:bg-muted/30" onClick={() => { setEdit(t); setOpen(true); }}>
                  <td className="px-4 py-3 font-medium">{t.title}</td>
                  <td className="px-4 py-3">{client ? <Badge variant="outline" style={{ borderColor: client.color ?? undefined }}>{client.name}</Badge> : <span className="text-muted-foreground">—</span>}</td>
                  <td className="px-4 py-3 text-muted-foreground">{assignee?.full_name || assignee?.email || "—"}</td>
                  <td className="px-4 py-3">{t.status ? <Badge variant="secondary">{statusLabels[t.status]}</Badge> : <span className="text-muted-foreground">—</span>}</td>
                  <td className="px-4 py-3">
                    {t.priority ? (
                      <Badge variant="outline" style={{ borderColor: priorityColors[t.priority], color: priorityColors[t.priority] }}>{priorityLabels[t.priority]}</Badge>
